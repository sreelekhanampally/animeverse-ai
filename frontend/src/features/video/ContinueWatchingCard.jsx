...
export function ContinueWatchingCard({ video, progress, onRemove }) {
    if (!video) return null;
    const { _id, title, thumbnail, duration, owner } = video;
    const to = `/watch/${_id}`;
    const hasProgress = Number.isFinite(progress) && progress > 0 && progress < 1;
    const pct = hasProgress ? Math.min(100, Math.max(4, Math.round(progress * 100))) : 0;
            ...
            <Link to={to} className="block">
                ...
                <div className="p-3">
                    ...
                    <div className="mt-0.5 flex items-center justify-between gap-3 text-[11px] text-muted">
                        ...
                        <span className="shrink-0">Recently watched</span>
                    </div>
                </div>
            </Link>
            {onRemove && (
